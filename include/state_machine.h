 ...
 *   - tick::nowMs()  : current tick timestamp
 *
 ...
 * all internal state.  Uses tick::nowMs() for the initial timestamp.
 *
 ...
 */
void start(float tempC = AMBIENT_START_C);
 ...
 */
void startDelay(uint32_t durationMs, State nextState = State::Idle);
 ...
 * @return          Number of faults entered within [tick::nowMs() - windowMs, tick::nowMs()].
 */
