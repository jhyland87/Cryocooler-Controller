...
bool QMI8658::begin(TwoWire &wire, uint8_t address) {
    _wire = &wire;
    _address = address;
    // NOTE: _wire->begin() intentionally removed.
    // The caller (hardware::init) is responsible for initialising the bus
    // with the correct SDA/SCL pins before begin() is called.  Calling
    // begin() on an already-running I2C driver returns ESP_ERR_INVALID_STATE
    // (259) on ESP-IDF 5.x and corrupts subsequent transactions.
...
bool QMI8658::readRegister(uint8_t reg, uint8_t *buffer, uint8_t length) {
    ...
    _wire->beginTransmission(_address);
    _wire->write(reg);
    // Use sendStop=true (full STOP then START) rather than sendStop=false
    // (repeated start).  On ESP-IDF 5.x the combined write-read path
    // (i2cWriteReadNonStop / i2c_master_transmit_receive) returns
    // ESP_ERR_INVALID_STATE; a plain STOP+START read is reliable and the
    // QMI8658 supports both access patterns.
