...
template <typename WIRE> class PCAL9535A {
  ...
  void begin(HardwareAddress addr) {
    ...
    // NOTE: mWire.begin() intentionally removed.  The shared Wire bus is
    // initialised once by hardware::Module::init() before any module runs.
    // Re-calling Wire.begin() here (ESP-IDF 5.x / Arduino Core 3.x) causes
    // ESP_ERR_INVALID_STATE on every subsequent I2C transaction because the
    // driver tries to re-initialise an already-running port.
